...
describe('lh memory - E2E', { timeout: TIMEOUT }, () => {
  const testDesc = `E2E-Memory-${Date.now()}`;
  let createdIdentityDeleted = false;
  let createdIdentityId: string;

  afterAll(() => {
    if (!createdIdentityId || createdIdentityDeleted) return;
    run(`memory delete identity ${createdIdentityId} --yes`);
  });
  ...
  describe('list', () => {
    ...
    it('should output JSON array for specific category', () => {
      ...
      expect(result).toContainEqual(
        expect.objectContaining({ description: testDesc, id: createdIdentityId }),
      );
    });

    it('should support JSON field filtering', () => {
      const result = runJson<any[]>('memory list identity --json id,description');
      expect(result).toContainEqual({ description: testDesc, id: createdIdentityId });
    });
  ...
  // Use an unknown task ID: querying an existing timed-out task can update its status.

  describe('extract-status', () => {
    it('should return empty for an unknown task ID', () => {
      const result = runJson<null>(`memory extract-status --task-id ${randomUUID()} --json`);
