...
Notes, reports, and reference material that an Agent creates and manages. Documents default to Agent-level scope, but can also link to the current topic. Uploaded source files belong to the Resource Library instead. See [Agent Documents](/docs/usage/agent/agent-documents).
...
Lets an Agent find current information on the internet and cite its sources. Click **+** below the Agent chat input, then enable **Web Search**. Choose **Smart Search** or **Provider Search**. **Off** disables Web Search. Provider Search can cause unexpected behavior and is not recommended. See [Web Search](/docs/usage/agent/web-search).
