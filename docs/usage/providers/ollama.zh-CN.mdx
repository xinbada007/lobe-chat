...
Ollama 是一款强大的本地运行大型语言模型（LLM）的框架，支持多种开源模型。现在，LobeHub 已经支持与 Ollama 的集成，这意味着你可以在 LobeHub 中轻松使用 Ollama 提供的语言模型来增强你的应用。
...
你可以在 LobeHub 的模型列表中查看 Ollama 当前可用的模型。首次选择尚未安装的模型时，页面会提示下载。
...
当然，你也可以通过在终端执行以下命令安装模型，以下以模型名称为例：
...
打开 **应用设置 → AI 服务商 → Ollama**，填写 Ollama 服务地址并打开服务商。点击 **检查** 测试连接后，可以在模型列表中获取、启用或添加本地模型。

如果 Ollama 运行在当前设备上，可以打开 **客户端请求模式**，让浏览器直接访问本地服务。
