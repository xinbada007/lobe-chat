...
Ollama is a powerful framework for running large language models (LLMs) locally. It supports a wide range of open models. LobeHub now integrates seamlessly with Ollama, allowing you to leverage these models directly within your chat interface.
...
Check the model list in LobeHub for models currently available from Ollama. When you select a model that is not installed, LobeHub prompts you to download it.
...
Alternatively, you can install models directly from the terminal by using the model name:
...
Open **App Settings → AI Providers → Ollama**, enter the Ollama server URL, and enable the provider. Select **Check** to test the connection. You can then fetch, enable, or add local models in the model list.

If Ollama runs on your current device, enable **Client Request Mode** so the browser can connect directly to the local service.
