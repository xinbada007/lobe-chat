...
Agent Group Chat lets several specialized Agents work on a task together. For example, a research Agent can gather information, an analysis Agent can review it, and a writing Agent can prepare the final document.
...
You can use a group to assign separate roles, compare perspectives, and have Agents review each other's work.
...
**Define clear roles** — Give each Agent a specific responsibility:
...
**Avoid redundancy** — Give each Agent a distinct role so the group spends less time repeating work.
...
**Right-size the team** — Start with 2–3 Agents and add more as needed. Larger teams can slow the workflow without improving results.
