...
## Uploaded Files and Agent Documents

Uploaded files belong to the Resource Library. Notes that an Agent creates for its own work belong to [Agent Documents](/docs/usage/agent/agent-documents). They have different lists and scopes. If an uploaded file is missing, search the Resource Library rather than the Agent document panel.

## Internal Search and Web Search

Internal search finds accessible Agents, conversations, files, and knowledge bases in LobeHub. [Web Search](/docs/usage/agent/web-search) retrieves information from external websites. Enabling web search does not make uploaded files public or replace knowledge-base retrieval.
...
- **AI-generated content** — Images created in the Image generation interface are saved automatically. View, download, and reuse them in conversations or drafts anytime.
