...
| `elasticsearch` | A larger deployment that needs BM25 search quality and can operate a dedicated search service and persistent sync job | Requires an Elasticsearch service (external, or the optional single-node service in the official Docker Compose file), an initial backfill, PostgreSQL change capture, and a continuously scheduled incremental consumer |
| `pg_like`       | A personal or small deployment on plain PostgreSQL that does not want to run Elasticsearch                            | None; substring matching runs directly against the existing tables without extensions, indexes, or sync workers                                                                                                          |
...
storage from transactional PostgreSQL, and when you can keep a sync job running continuously.
Personal and small deployments whose database provider is ending `pg_search` should consider the
service-free `pg_like` backend first.

Neon stopped offering `pg_search` to new projects on March 19, 2026. Retirement timelines for
existing projects follow the notice Neon sends for that project. LobeHub users who receive such a
notice should use
[Migrate from pg\_search on Neon](/docs/self-hosting/advanced/neon-pg-search-migration) to choose
`pg_like` or Elasticsearch. Check Neon's
[current pg\_search notice](https://neon.com/docs/extensions/pg_search) before acting.
...
See [Migrate from pg\_search on Neon](/docs/self-hosting/advanced/neon-pg-search-migration) for
guidance on choosing a backend and starting the migration.
...
### Rebuild the current Elasticsearch schema generation

When a projection or capture behavior change requires every historical document to be regenerated,
first upgrade every application and continuous-sync worker. Then build a new physical generation
from PostgreSQL without changing the mapping version:
...
The command prints a run ID and an index such as `<namespace>-messages-v1-r<run-id>`. If interrupted,
run the same command with `--run-id=<run-id>`. Continuous sync writes both the existing and rebuilding
generations; do not pause application writes or the sync worker. Creating the checkpoint briefly
fences only the PostgreSQL tables that can emit the selected entities; resuming the same checkpoint
does not repeat that fence. After the backfill reports
`ready_for_incremental_sync` and the `messages` Outbox is idle, promote the exact physical index:

```bash
docker compose run --rm fts-search-reindex \
  --promote --entity=messages --generation=<exact-index> --yes
```

The alias and logical schema version remain unchanged. Keep the previous generation during an
observation window so the alias can be promoted back to its exact name. Once rollback is no longer
needed, run `--purge --entity=messages --yes`. Purge installs an exact-name auto-create guard before
deleting each detached old index. Self-managed clusters may run `--retire --entity=messages --yes`
first to close old indexes; Elastic Cloud Serverless does not support index close, so skip retire and
purge directly.
