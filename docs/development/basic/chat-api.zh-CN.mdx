---
title: Chat API 内部实现与前后端交互
description: 面向源码开发者，说明 LobeHub 内部聊天链路、运行时及前后端交互；不是公开 API 接入指南。
tags:
  - Chat API
  - 前后端交互
  - Model Runtime
...
下方时序图主要展开客户端 Agent 循环。服务端执行另见[客户端 vs 服务端执行](#客户端-vs-服务端执行)。不同 transport 和执行场景不应被当作完全相同的链路。
...
本节描述的客户端 Agent 聊天链路通过 `AgentRuntime.step()` 循环驱动。下文的预设任务直接调用模型，不经过该循环。
