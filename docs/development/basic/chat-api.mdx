---
title: Chat API Internals and Client-Server Interaction
description: >-
  Understand LobeHub internal chat flows, runtimes, and client-server
  interactions. This is a source development guide, not a public API tutorial.
tags:
  - Chat API
...
The diagram below focuses on the client-side Agent loop. See [Client-Side vs Server-Side Execution](#client-side-vs-server-side-execution) for server execution. Different transports and execution scenarios do not necessarily follow an identical path.
...
The client-side Agent chat flow described here is driven by the `AgentRuntime.step()` loop. Preset tasks described below call the model directly and bypass this loop.
